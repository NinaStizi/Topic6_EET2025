...
folder1 = "C:/Users/Marion/Dropbox/Research/Teaching/2023-2024/exam/pollution/greenstone 2014/Greenstone-and-Hanna--2014--Replication-Files/for students water/data/" # Your folder name ending with a /
...
loc = paste(folder1,"R-ready_","bod",".dta", sep = "")

print(paste("Pollutant : ", "bod")) #Tells the advancement of the loop
A = read_dta(loc)
...
reprod_scratch = lm(C[["bod"]]~D+controls+year+cityriver, weights = A$pop_urban)
...
# /!\ Do not \n mid next line !
strform=paste("bod"," ~ taum7 + taum6  + taum5 +taum4 +taum3 + taum2 + taum1 + tau0+ tau1 + tau2 + tau3 + tau4 + tau5 + tau6 + tau7 + tau8 + tau9 + tau10 + tauL + tauR+ lit_urban + pce | year + cityriver")
...
stata_pol = read_dta(paste(folder1, "sigmas_","bod",".dta", sep =""))
...
texreg(list(a2paper_reproduced, b2paper_reproduced, c2paper_reproduced,a2paper_real,b2paper_real,c2paper_real),custom.model.names = paste("fit", 1:6),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
       ...
       include.adjrs = FALSE, caption = paste("Paper reproduction:  Estimates of the Effect of the NRCP on ", "bod",".", sep = ""), caption.above =TRUE,
       custom.note = "%stars. Methodology",file = paste(folder,"LaTex_Paper_","bod",".txt",sep = ""))
...
strformNoCtrl=paste("bod"," ~ taum7 + taum6  + taum5 +taum4 +taum3 + taum2 + taum1 + tau0+ tau1 + tau2 + tau3 + tau4 + tau5 + tau6 + tau7 + tau8 + tau9 + tau10 + tauL + tauR| year + cityriver")
...
texreg(list(a2, b2, c2, a2NC, b2NC, c2NC),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
       ...
       include.adjrs = FALSE, caption = paste("Trend Break Estimates of the Effect of the NRCP on ", "bod",".", sep = ""), caption.above =TRUE,
       custom.note = "%stars. COPY-PASTE note from paper please !",file = paste(folder,"LaTex_","bod",".txt",sep = ""))
...
# Regression itself :
strformSA=paste("bod","~lit_urban +pce + sunab(cohort, year)| cityriver + year")
...
strformSANC=paste("bod","~sunab(cohort, year)| cityriver + year")
...
texreg(list(a2SA, b2SA, c2SA, a2SANC, b2SANC, c2SANC),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
       omit.coef = "(cityriver)|(year)",stars = c(0.01, 0.05, 0.1),reorder.gof = c(5,4,3,2,1),custom.gof.rows = list("Equation fitted"=rep(c("2a","2b","2c"),2),"Multi-way clustering in first stage" = rep("yes",6),
                                                                                                                     "Controls in first stage" = c(rep("No",3),rep("Yes",3))),
       include.adjrs = FALSE, caption = paste("Extension - Trend Break Estimates of the Effect of the NRCP on ", "bod",".", sep = ""), caption.above =TRUE,
       custom.note = "%stars. This table contains 2nd step regressions of our extension with Sun and Abraham method.",file = paste(folder,"SunAbraham_","bod",".txt",sep = ""))
#Plot estimated effects
png(filename=paste(folder,"iplot_","bod",".png", sep =""),width =500,height = 400 )
