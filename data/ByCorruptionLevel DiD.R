# This script computes mimics all the outputs of "Diff in Diff.R" after
# restricting the dataset to different levels of corruption.
# as a measure of pollution, but this can be changed (lines 15 and 16, and ylim parameter line 207)
...
# fixed effect regressions from scratch
library(coefplot)     # To easily plot Sun & Abraham method outputs
library(texreg)       # To convert fitted models to LaTex arrays.


folder_base = "YourPath/"
pollutants  = c("bod", "lnfcoli", "do")
for (pol in pollutants){
loc = paste(folder_base,"R-ready_",pol,".dta", sep = "")
Dataset = read_dta(loc)
# Folder for output by corruption level
dir.create(file.path(folder_base, paste("OutputsByCorruptionLevel_",pol, sep = "")), showWarnings = TRUE)
folder = paste(folder_base,"OutputsByCorruptionLevel_",pol,"/", sep ="")
# Loop over corruption level, for a given pollution measure
for (cor_level in c("Moderate","Alarming","High","Very High")){
  print(paste("Corruption level : ", cor_level)) #Tells the advancement of the loop
  A = Dataset[Dataset$corruption_level07==cor_level,]
  ...
  stata_pol = read_dta(paste(folder_base, "sigmas_",pol,".dta", sep =""))
  ...
  texreg(list(a2paper_reproduced, b2paper_reproduced, c2paper_reproduced,a2paper_real,b2paper_real,c2paper_real),custom.model.names = paste("fit", 1:6),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
         ...
         include.adjrs = FALSE, caption = paste("Paper reproduction:  Estimates of the Effect of the NRCP on ", pol," for corruption level: ",cor_level,".", sep = ""), caption.above =TRUE,
         custom.note = "%stars. Methodology",file = paste(folder,"LaTex_Paper_",pol,"_coruptLvl_,",cor_level,".txt",sep = ""))
  ...
  # Multi-Way-Clustering : River City Year
  reprod_builtin2w = feols(form, data = A, weights = A$pop_urban, cluster = c("city","river"))
  
  # Multi-Way-Clustering : River City Year, no controls
  ...
  texreg(list(a2, b2, c2, a2NC, b2NC, c2NC),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
         ...
         include.adjrs = FALSE, caption = paste("Trend Break Estimates of the Effect of the NRCP on ", pol," for corruption level: ",cor_level,".", sep = ""), caption.above =TRUE,
         custom.note = "%stars. COPY-PASTE note from paper please !",file = paste(folder,"LaTex_",pol,"_coruptLvl_,",cor_level,".txt",sep = ""))
  ...
  SunAbr = feols(formSA, C, weights = C$pop_urban, cluster = c("river", "city"))
  ...
  SunAbrNoCtrl = feols(formSANC, C, weights = C$pop_urban, cluster = c("river","city"))
  ...
  texreg(list(a2SA, b2SA, c2SA, a2SANC, b2SANC, c2SANC),custom.coef.map = list("stata_pol$tau" = "time trend","stata_pol$nrcp"="1(policy)", "stata_pol$nrcp_trend"="1(policy)*trend"),
         omit.coef = "(cityriver)|(year)",stars = c(0.01, 0.05, 0.1),reorder.gof = c(5,4,3,2,1),custom.gof.rows = list("Equation fitted"=rep(c("2a","2b","2c"),2),"Multi-way clustering in first stage" = rep("yes",6),
                                                                                                                       "Controls in first stage" = c(rep("No",3),rep("Yes",3))),
         include.adjrs = FALSE, caption = paste("Extension - Trend Break Estimates of the Effect of the NRCP on ", pol," for corruption level: ",cor_level,".", sep = ""), caption.above =TRUE,
         custom.note = "%stars. This table contains 2nd step regressions of our extension with Sun and Abraham method.",file = paste(folder,"SunAbraham_",pol,"_coruptLvl_,",cor_level,".txt",sep = ""))
  #Plot estimated effects
  png(filename=paste(folder,"iplot_",pol,"_coruptLvl_",cor_level,".png", sep =""),width =500,height = 400 )
  iplot(SunAbr, xlim=c(-7,10), ylim = c(-2,2),main = "", xlab = "Year (lag)", ylab = "Estimated effect and 95% Conf.Int")
  dev.off()
}
}
